...
{                                         // Receives serial bits from the roaster and stores them in the receive buffer.
  ...
  while (millis() - startTime < TIMEOUT_PREAMBLE_SEARCH)
  {
    ...
    if (pulseDuration >= (preamble - 500) && pulseDuration <= (preamble + 1000))
    {                          // Check that the pulse is approximately 7.5ms
...
void getArtisanMessage()
{
  if (Serial.available() > 0)
  {
    ...
    { // Hanlde the TC4 init message
      handleCHAN();
    }
    else if (command == "UNITS")
    {
      if (split >= 0)
        CorF = input.charAt(split + 1);
    }
  }
}
...
bool itsbeentoolong()
{ // Checks if too much time has passed since the last control message
...
{ // Checks if the current temperature exceeds the maximum allowed temperature in celcius
  if (tempC > maxTemp)
  {
    return true;
  }
